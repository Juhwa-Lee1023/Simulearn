  ...
  const triggerHelp = useCallback(() => {
    ...
    if (step === 'level-1-task') {
      if (reviewStage === 'designer') {
        tip = {
          title: "기획자가 정의할 것",
          concept: "디자이너에게 '무엇을 보여줄지'만 알려주세요. '어떻게 보여줄지'는 디자이너가 결정합니다.",
          guide: "예: '매진 임박 매장에 남은 수량 표시', '매장명과 함께 강조 문구 노출' - 색상/폰트/레이아웃은 적지 않아도 됩니다."
        };
      } else if (reviewStage === 'developer') {
        tip = {
          title: "기획자가 정의할 것",
          concept: "개발자에게 '문제 상황에서 사용자에게 뭘 보여줄지'만 알려주세요. 기술적 구현은 개발자가 결정합니다.",
          guide: "예: '데이터 로딩 실패 시 \"잠시 후 다시 시도해주세요\" 메시지 노출' - API 설계나 에러 코드는 적지 않아도 됩니다."
        };
      } else {
        tip = {
          title: "기획자가 정의할 것",
          concept: "QA에게 '기능 완료의 기준'만 알려주세요. 테스트 시나리오 작성은 QA가 합니다.",
          guide: "예: '재고 10% 이하면 매진 임박 표시', '재고 0%면 품절 표시' - 구체적인 숫자 기준만 정의하면 됩니다."
        };
      }
    } else {
      tip = {
        title: "명확한 의사결정",
        concept: "팀원들은 기획자의 결정을 기다립니다.",
        guide: "매진 임박을 어디에 표시할지(지도/리스트/둘 다) 결정해서 알려주세요."
      };
    }
